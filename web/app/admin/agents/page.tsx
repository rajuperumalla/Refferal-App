...
function AgentModal({ onClose, onSave, existing, agentCount }: {
  ...
  const [form, setForm] = useState({
    ...
    commissionRate: existing?.commissionRate ?? 5,
    bank: existing?.bank ?? '', upi: existing?.upi ?? '',
    specialties: existing?.specialties ?? [],
    status: existing?.status ?? 'pending' as AgentStatus,
  });
  const [commRateRaw, setCommRateRaw] = useState(String(existing?.commissionRate ?? 5));
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                Commission Rate (%) *
                <span className="ml-1.5 text-xs text-gray-400 font-normal">5 – 25</span>
              </label>
              <div className="relative">
                <input
                  ...
                  onChange={e => {
                    ...
                    if (!isNaN(v)) set('commissionRate', v);
                  }}
                  onBlur={() => {
                    const v = parseFloat(commRateRaw);
                    const clamped = isNaN(v) ? 5 : Math.min(25, Math.max(5, v));
                    setCommRateRaw(String(clamped));
                    set('commissionRate', clamped);
                  }}
                  placeholder="e.g. 10"
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-semibold text-gray-400">%</span>
              </div>
              <p className="text-[11px] text-gray-400 mt-1">Enter a value between 5% and 25%</p>
            </div>
            <div>
...
export default function AgentsPage() {
  ...
  const statusCounts: Record<string, number> = { all: onlyAgents.length };
  onlyAgents.forEach(a => { statusCounts[a.status] = (statusCounts[a.status] ?? 0) + 1; });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Agents',     value: onlyAgents.length,            color: 'text-gray-900' },
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search agents by name, ID, city or email…"
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-100 text-sm text-gray-500">
          Showing <strong className="text-gray-900">{filtered.length}</strong> of {onlyAgents.length} agents
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                {['Agent','ID','City','Manager','Commission','Leads','This Month','Conv.%','Status','Last Active','KYC','Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.length === 0 ? (
                <tr><td colSpan={12} className="px-5 py-16 text-center text-gray-400"><div className="text-4xl mb-2">👥</div><div className="font-medium">No agents found</div></td></tr>
              ) : filtered.map(a => {
                  ...
                  <tr key={a.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-sm text-indigo-700 flex-shrink-0">{a.name[0]}</div>
                        <div>
